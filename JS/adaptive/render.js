...
function render() {
  ...
  drawPaddle(state.ai, aiPaddleColor());
...
function spawnParticle(W, H, randomY = false) {
  ...
  return {
    ...
    color:         Math.random() < 0.5 ? 'adaptive-secondary' : 'adaptive-primary',
...
function drawParticles(W, H) {
  ...
  for (let i = 0; i < state.particles.length; i++) {
    ...
    const col = p.color === 'adaptive-secondary'
...
function drawCollisionParticles() {
  ...
  for (let i = state.collisionParticles.length - 1; i >= 0; i--) {
    ...
    // Glow — same color as the fragment, matches the overall neon feel
    ctx.shadowColor = p.color;
    ctx.shadowBlur  = 6 + (1 - lifeFrac) * 0; // subtle, doesn't bloom

    ctx.translate(p.x, p.y);
    ctx.rotate(p.rotation);

    if (p.shape === 'rect') {
      // Paddle fragment — rectangle debris
      ctx.fillRect(-p.rectW * 0.5, -p.rectH * 0.5, p.rectW, p.rectH);
    } else {
      // Ball fragment — uses same drawShape helper as the ball itself
...
function drawGoalPulses(W, H) {
  ...
  for (const side of ['player', 'ai']) {
    ...
    const col      = side === 'player' ? aiPaddleColor() : playerColor();
    const gradW    = W * 0.52;
...
function playerColor() {
  return getComputedStyle(document.documentElement)
    .getPropertyValue('--color-player').trim() || '#ffffff';
}
